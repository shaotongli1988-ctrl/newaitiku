...
- 构建通过: `按模块执行对应 build 命令`
- 单元测试通过: `pytest -q` (或受影响模块最小测试集)
- 静态检查通过: `按模块执行 lint/typecheck`
...
| P1-01 | P1 | test | 为连续循环补充标准验证命令清单 | 在 TODO 的 DoD 中填充项目可执行命令 | P0-03 | todo | 待补充 |
| P1-02 | P1 | docs | 沉淀阻塞问题模板和决策 SLA | Blockers 表至少含 1 条示例和处理规则 | P1-01 | todo | 待补充 |
| P1-03 | P1 | chore | 收敛守卫高优先告警（API 契约漂移与复用守卫噪音） | 在不依赖 break-glass 的情况下通过 `unified_delivery_guard --phase batch/final` | P0-03 | todo | `BG-303` 临时豁免记录 |
